...
def clean_for_summary(value: Any) -> str:
    text = safe_display(value).strip()
    return text if text else "VACÍO"


def build_change_summary(tipo: str, old: Any, new: Any) -> str:
    return f"{tipo}: {clean_for_summary(old)} → {clean_for_summary(new)}"


def build_order_summary(row: dict[str, Any]) -> str:
    parts = [
        f"Buque: {clean_for_summary(row.get('buque', row.get('vessel', '')))}",
        f"Fecha: {clean_for_summary(row.get('fecha', row.get('arrival_date', '')))}",
        f"Tonelaje: {clean_for_summary(row.get('tonelaje', row.get('tonnage', '')))}",
        f"Carga: {clean_for_summary(row.get('carga', row.get('cargo', '')))}",
        f"Puerto: {clean_for_summary(row.get('puerto_descarga', row.get('discharge_port', '')))}",
        f"Importador: {clean_for_summary(row.get('importador', row.get('importer', '')))}",
    ]
    return " | ".join(parts)


def build_review_hint(tipo: str, field: str, old: Any, new: Any, risk: str) -> str:
    risk_text = f"Riesgo {risk}" if risk else "Revisar"
    return f"{risk_text}. Validar {field}: {clean_for_summary(old)} → {clean_for_summary(new)}"
...
def make_change_row(isa_row, ext_row, field, tipo, old, new, risk, action, reason, confidence, criterion) -> dict[str, Any]:
    resumen_cambio = build_change_summary(tipo, old, new)
    return {
        "grupo_buque": normalize_text(ext_row.get("vessel", "")) or normalize_text(isa_row.get("vessel", "")),
        "cambio_detectado": resumen_cambio,
        "revision_recomendada": build_review_hint(tipo, field, old, new, risk),
        "buque_isaosa": safe_display(isa_row.get("vessel", "")),
        "buque_externo": safe_display(ext_row.get("vessel", "")),
        "campo": field,
        "antes_isaosa": safe_display(old),
        "despues_externo": safe_display(new),
...
def build_new_rows_from_block(external_block: dict[str, Any], score: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for ext_row in external_block["rows"]:
        ...
        row["pedido_detectado"] = build_order_summary(row)
        row["revision_recomendada"] = "Validar si este viaje debe agregarse a ISAOSA."
...
def reorder_columns_for_review(df: pd.DataFrame, key_prefix: str) -> pd.DataFrame:
    if df is None or df.empty:
        return df

    if key_prefix == "changes":
        priority = [
            "decision_usuario",
            "cambio_detectado",
...
def render_quick_change_cards(df: pd.DataFrame, key_prefix: str) -> None:
    if df is None or df.empty:
        return

    if key_prefix == "changes" and "cambio_detectado" in df.columns:
        st.markdown("#### Cambios principales")
        preview_cols = [
            c for c in [
                "decision_usuario",
                "cambio_detectado",
        ...
        st.dataframe(df[preview_cols].head(25), use_container_width=True, hide_index=True)
    elif key_prefix == "new" and "pedido_detectado" in df.columns:
        st.markdown("#### Pedidos/viajes nuevos detectados")
        preview_cols = [
            c for c in [
                "decision_usuario",
                "pedido_detectado",
...
def render_grouped_editor(
    ...
    render_quick_change_cards(df, key_prefix)

    edited_parts = []
    search = st.text_input("Buscar buque, campo, valor o importador", key=f"{key_prefix}_search").strip().upper()
    ...
    for group_name, group_df in groups:
        ...
        if key_prefix == "changes" and "cambio_detectado" in group_df.columns:
            summary_text = " · ".join(group_df["cambio_detectado"].astype(str).head(3).tolist())
            if len(group_df) > 3:
                summary_text += f" · +{len(group_df)-3} más"
            expander_title = f"🚢 {label} · {len(group_df)} cambio(s) · {summary_text}"
        elif key_prefix == "new" and "pedido_detectado" in group_df.columns:
            expander_title = f"🚢 {label} · {len(group_df)} línea(s) nueva(s)"
        else:
            expander_title = f"🚢 {label} · {len(group_df)} registro(s)"

        with st.expander(expander_title, expanded=len(groups) <= 3):
            ...
            with left:
                ...
                if key_prefix == "changes":
                    risk_counts = group_df["nivel_riesgo"].value_counts().to_dict() if "nivel_riesgo" in group_df.columns else {}
                    st.caption("Riesgos: " + ", ".join(f"{k}: {v}" for k, v in risk_counts.items()) if risk_counts else "Sin riesgos clasificados")

            with right:
                display_df = reorder_columns_for_review(group_df.copy(), key_prefix)
                edited = st.data_editor(
                    ...
                    column_config={
                        ...
                        "cambio_detectado": st.column_config.TextColumn(
                            "Cambio detectado",
                            help="Resumen claro de lo que cambió: valor ISAOSA → valor externo.",
                        ),
                        "pedido_detectado": st.column_config.TextColumn(
                            "Pedido/viaje detectado",
                            help="Resumen del buque nuevo que podría agregarse.",
                        ),
                        "revision_recomendada": st.column_config.TextColumn(
                            "Revisión recomendada",
                            help="Qué validar antes de aceptar/agregar.",
                        ),
                    },
                    disabled=[c for c in display_df.columns if c not in {"decision_usuario"}],
                )
    ...
    if search and "row_id" in df.columns and "row_id" in edited_all.columns:
...
def main():
    ...
    with tab_new:
        ...
        st.caption("Aquí se ve primero el resumen del pedido/viaje detectado. Marca AGREGAR solo si quieres insertarlo en el ISAOSA actualizado.")
    ...
    with tab_changes:
        ...
        st.caption("Aquí se muestra de forma directa qué cambió: valor en ISAOSA → valor externo. Marca ACEPTAR solo si quieres aplicar ese cambio.")
